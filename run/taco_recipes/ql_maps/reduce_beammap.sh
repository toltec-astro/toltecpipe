#!/usr/bin/env bash
# reduce_beammap.sh — v3 quick-look beammap/azscan/elscan reduction.
#
...
#
# Optional env vars:
#   TOLTECA_RCDIR           — path to tolteca run config dir with beammap.yaml
#                             (defaults to same dir as this script)
#
# Outputs to: ${TOLTECA_DATAPROD_ROOT}/${obsnum}/
#   toltec_a1100_beammap_<obsnum>.fits
#   toltec_a1400_beammap_<obsnum>.fits
#   toltec_a2000_beammap_<obsnum>.fits
#   toltec_a1100_beammap_<obsnum>.png
#   ...
...
echo "[toltecpipe] reduce_beammap.sh obsnum=${OBSNUM}"
...
# --- Step 3: run citlali ---
echo "[toltecpipe] Running citlali (beammap) for obsnum=${OBSNUM}..."
CITLALI_OUTDIR="${OUTDIR}/citlali"
mkdir -p "${CITLALI_OUTDIR}"

"${CITLALI}" \
    --config "${RCDIR}/beammap.yaml" \
    --input_dir "${WORKDIR}" \
...
echo "[toltecpipe] reduce_beammap.sh done → ${OUTDIR}"
